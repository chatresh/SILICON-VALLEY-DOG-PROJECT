...
backgroundsound = loadSound("sounds/bg.mp3");
dogfeed = loadSound("sounds/Dog Woof-SoundBible.com-457935112.mp3");
gameover = loadSound("sounds/gameOver.mp3");
coincollect  =loadSound("sounds/jump.mp3");
}
...
outmaze15.shapeColor = "black";
 ...
 // coins for buying food
  ...
  dogFood = createSprite(140,150,20,20);
  dogFood.addImage(foodimg);
  dogFood.scale = 0.4;
...
//giving feedback to game
 textSize(30);
  ...
  textSize(15);
  text("click on the food to feed the dog",50,210);
  
if(gameState===0){
  textSize(20);
  text("press space to start the game",600,500);
...
// if we win
if(player.x<200 && mousePressedOver(dogFood)&&coins===50){
  gameState = 2;
  }

 
   if( gameState === 2 && dogs!== undefined){
      dogs[flag].destroy();     
      gameState = 0;
      player.x =1130;
      player.y = 550;
  ...
  coin5.scale = 0.5;
   ...
   {
     textSize(80);
     text("G A M E C O M P L E T E D ",250,350);
     man1.destroy();
     man2.destroy();
     man3.destroy();
     man4.destroy();
 ...
 player.destroy();
  ...
  lfb.position(650,180);
  

   var resetbutton = createButton('reset');
   resetbutton.position(700,280);
   resetbutton.size(150);
    gameover.play();
